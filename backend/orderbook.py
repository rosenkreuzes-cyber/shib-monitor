...
class OrderBookEngine:
    def __init__(self, max_events=5000):
        ...
        self.last_sequence=None; self.ready=False; self.last_snapshot_ts=None
    def load_snapshot(self,bids,asks,sequence=None):
        self.bids={float(p):float(a) for p,a in bids if float(a)>0}
        self.asks={float(p):float(a) for p,a in asks if float(a)>0}
    ...
    def walls(self,pct=.005,multiple=6.):
        ...
        for side,book in (("buy",self.bids),("sell",self.asks)):
            ...
            for p,a in book.items():
                if abs(p-m)/m<=pct and base>0 and a>=base*multiple:
    ...
    def snapshot(self):
        ...
        return {"ready":self.ready,"best_bid":self.best_bid(),"best_ask":self.best_ask(),"mid":self.mid(),
                "imbalance_pct":round(im,1),"weighted_bid":b,"weighted_ask":a,"walls":self.walls(),
                "book_change":self.change_pressure(),"sequence":self.last_sequence,
