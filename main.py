...
def health_payload():
    ...
    return {
        ...
        "ws_ticker_messages": getattr(stream, "ws_ticker_messages", 0) if stream else 0,
        "ws_subscribe_messages": getattr(stream, "ws_subscribe_messages", 0) if stream else 0,
        "ws_error_messages": getattr(stream, "ws_error_messages", 0) if stream else 0,
        "ws_reconnects": getattr(stream, "ws_reconnects", 0) if stream else 0,
        "last_ws_trade_ts": getattr(stream, "last_ws_trade_ts", None) if stream else None,
        "last_ws_ticker_ts": getattr(stream, "last_ws_ticker_ts", None) if stream else None,
        "last_pong_ts": getattr(stream, "last_pong_ts", None) if stream else None,
        "last_subscribe_ts": getattr(stream, "last_subscribe_ts", None) if stream else None,
        "last_seq_id": getattr(stream, "last_seq_id", None) if stream else None,
        "last_prev_seq_id": getattr(stream, "last_prev_seq_id", None) if stream else None,
        "last_checksum": getattr(stream, "last_checksum", None) if stream else None,
        "last_action": getattr(stream, "last_action", None) if stream else None,
        "ws_raw_preview": getattr(stream, "last_raw_preview", None) if stream else None,
        "ws_last_error": getattr(stream, "last_ws_error", None) if stream else None,
        "ws_trade_messages": (
