...
async def rest_fallback_runner():
    """Keep the UI/data live when Coincheck WebSocket stalls.

    REST is only used when the local order book has not received a fresh
    order-book message recently. This prevents the page from reaching
    判定停止 simply because Render's WebSocket connection is quiet.
    """
    while True:
        try:
            ...
            age = analyzer.book.data_age_sec()

            if age is None or age > REST_FALLBACK_SECONDS:
                LOG = logging.getLogger(__name__)
                LOG.warning(
                    "REST fallback refresh pair=%s ws_connected=%s "
                    "orderbook_age=%s",
                    PAIR,
                    getattr(stream, "connected", False),
                    age,
                )
            ...
            logging.getLogger(__name__).exception(
                "REST fallback runner failed"
            )


@asynccontextmanager
async def lifespan(app):
    stream_task = asyncio.create_task(stream_runner())
    fallback_task = asyncio.create_task(rest_fallback_runner())
    ...
    try:
        ...
        for task in (stream_task, fallback_task):
            try:
                await task
            except asyncio.CancelledError:
                pass
...
async def refresh():
    if stream is None:
        return {"ok": False, "error": "stream_not_ready", "api_version": VERSION}

    await stream.rest_snapshot()
    data = analyzer.snapshot()
    data["api_version"] = VERSION
    return {"ok": True, "api_version": VERSION, "data": data}
